...
# Verbose mode: show full colorized tracebacks
pyforge -v
pyforge --verbose
...
Failed and errored tests show a compact `→ file:line  in func` pointer so you can jump straight to the failing line:
...
test_example.py
  ...
  ERROR test_api_endpoint (Line 30): name 'api_client' is not defined
    → tests/test_example.py:33  in test_api_endpoint
  SKIPPED test_local_only (Line 40): Skip in CI environment

------------------------------------------------------------------------
Summary: PASSED: 4/7  FAILED: 1/7  SKIPPED: 1/7  ERRORS: 1/7
Took 156 ms to execute all tests
------------------------------------------------------------------------
```

#### Verbose mode (`pyforge -v`)

Failed and errored tests show a full colorized traceback. Internal pyforge frames are automatically filtered out — only your code appears:
...
#### Quiet mode (`pyforge -q`)

Only failures, errors, and the summary are shown. Passed and skipped tests are hidden.
...
- Compact location `→ file:line  in func` for failures/errors in normal mode
- Full colorized traceback in verbose mode
...
| `--verbose`   | `-v`  | Show full colorized tracebacks   | `pyforge -v`              |
| `--fail-fast` |       | Stop at first failure            | `pyforge --fail-fast`     |
| `-k`          |       | Filter by test name substring    | `pyforge -k basic`        |
...
```python
# Recommended: Import from the main package
from pyforge_test import (
    test,
    test_marker,
    test_parameterized,
...
# Alternative: Import directly from core (same symbols)
...
6. **Clean tracebacks** — framework internals filtered out, compact location in normal mode, full colorized output in verbose mode
